...
export default function LivePreview({ state }: { state: LayoutDividerState }) {
  ...
  const lineStyle: CSSProperties = { width: isVertical ? state.thickness : state.length, height: isVertical ? state.length : state.thickness, borderRadius: 999, background: state.accent };
